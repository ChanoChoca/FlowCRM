...
import com.flashpage.app.comunicacion.repository.AnuncioRepository;
...
public class AnuncioService {
    ...
    public AnuncioResponse publicar(CreateAnuncioRequest req, Usuario autor) {
        ...
        if (req.rolesDestino() != null) anuncio.setRolesDestino(req.rolesDestino());

        Anuncio saved = anuncioRepo.save(anuncio);
        List<Rol> destinos = saved.getRolesDestino().isEmpty() ? List.of(Rol.values()) : saved.getRolesDestino();
        ...
        usuarioRepo.findAll().stream()
                ...
                .forEach(u -> {
                    notificacionService.crear(u.getId(), TipoNotificacion.ANUNCIO,
                            "Nuevo anuncio: " + saved.getTitulo(), autorNombre + " publicó un comunicado.", saved.getId());
                    emailService.enviarAnuncio(u.getEmail(), u.getNombre(), autorNombre, saved.getTitulo(), saved.getContenido());
                });
    ...
    public AnuncioResponse marcarLeido(Long id, Usuario usuario) {
        Anuncio anuncio = anuncioRepo.findById(id).orElseThrow(() -> new NotFoundException("Anuncio no encontrado"));
    ...
    public void eliminar(Long id, Usuario usuario) {
        Anuncio anuncio = anuncioRepo.findById(id).orElseThrow(() -> new NotFoundException("Anuncio no encontrado"));
        if (!anuncio.getAutor().getId().equals(usuario.getId()) && usuario.getRol().valor < Rol.JEFE_DE_SUPERVISOR.valor) {
