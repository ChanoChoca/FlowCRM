...
public final class VentaSpecification {
    ...
    public static Specification<Venta> activa() {
        return (root, query, cb) -> cb.not(
                root.get("estado").in(EstadoVenta.CANCELADA, EstadoVenta.RECHAZADA));
    }
    ...
    public static Specification<Venta> productoNombreContains(String producto) {
        return (root, query, cb) -> cb.like(cb.lower(root.get("producto").get("nombre")),
                "%" + producto.toLowerCase() + "%");
    }

    public static Specification<Venta> centralNombreContains(String central) {
        return (root, query, cb) -> cb.like(cb.lower(root.get("central").get("nombre")),
                "%" + central.toLowerCase() + "%");
    }
    ...
    public static Specification<Venta> creadoDesde(LocalDate desde) {
        return (root, query, cb) -> cb.greaterThanOrEqualTo(root.get("creadoEn"),
                desde.atStartOfDay());
    }

    public static Specification<Venta> creadoHasta(LocalDate hasta) {
        return (root, query, cb) -> cb.lessThan(root.get("creadoEn"),
